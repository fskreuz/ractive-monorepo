...
import { toParts, toCJS } from '@ractivejs/utils-component'
...
program
  ...
  .action(() => {
    readFromStream(process.stdin).then(partsString => {
      return writeToStream(process.stdout, toCJS({ srcName: '', destName: '', parts: JSON.parse(partsString) }).code)
    }).catch(err => {
      console.error(err.message)
      process.exit(1)
    })
...
program
  ...
  .action(cmd => {
    ...
    readFromStream(process.stdin).then(partsString => {
      ...
      return writeToStream(process.stdout, JSON.stringify(toCJS({ srcName, destName, parts }).map))
    }).catch(err => {
      console.error(err.message)
      process.exit(1)
    })
