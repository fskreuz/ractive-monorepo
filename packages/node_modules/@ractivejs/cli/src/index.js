...
import { toParts, toCJS, toESM, isESM } from '@ractivejs/utils-component'
...
program
  ...
  .action(() => {
    readFromStream(process.stdin).then(partsString => {
      ...
      return writeToStream(process.stdout, moduler({ srcName: '', destName: '', parts }).code)
    }).catch(err => {
      console.error(err.message)
      process.exit(1)
    })
...
program
  ...
  .action(cmd => {
    ...
    readFromStream(process.stdin).then(partsString => {
      const parts = JSON.parse(partsString)
      const moduler = isESM((parts.script && parts.script.code) || '') ? toESM : toCJS
      return writeToStream(process.stdout, JSON.stringify(moduler({ srcName, destName, parts }).map))
    }).catch(err => {
      console.error(err.message)
      process.exit(1)
    })
