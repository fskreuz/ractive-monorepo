...
const componentNamePattern = /^[a-zA-Z$_][a-zA-Z0-9$_-]*$/
const componentExtensionPattern = /(?:\.ractive)?\.html$/
const esmPattern = /(^\s*|[});\n]\s*)(import\s*(['"]|(\*\s+as\s+)?(?!type)([^"'()\n; ]+)\s*from\s*['"]|\{)|export\s+\*\s+from\s+["']|export\s*(\{|default|function|class|var|const|let|async\s+function))/

export const isESM = source => esmPattern.test(source)

export const replaceExtension = (module, ext) => module.replace(componentExtensionPattern, ext)

export const isComponentPath = path => componentExtensionPattern.test(path)
...
export const getRequiredModules = string => {
  ...
  // TODO: Edge case, this matches method calls named require (i.e. foo.require())
