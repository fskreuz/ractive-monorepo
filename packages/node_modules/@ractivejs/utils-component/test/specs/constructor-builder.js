...
QUnit.module('constructor-builder')

QUnit.test(`duplicate-link-href`, assert => {
  return fetch(`/base/test/samples/cjs/duplicate-link-href/component.ractive.html`).then(r => r.text()).then(source => {
    const dependencies = {
    ...
    const resolver = id => dependencies[id]
    const constructor = toConstructor({ srcName, destName, parts, resolver })
...
QUnit.test(`duplicate-link-name`, assert => {
  return fetch(`/base/test/samples/cjs/duplicate-link-name/component.ractive.html`).then(r => r.text()).then(source => {
    const dependencies = {
      './path/to/MyComponent.ractive.html': Ractive.extend({ template: '<div class="my-component">My Component</div>' }),
...
QUnit.test(`empty-top-levels`, assert => {
  return fetch(`/base/test/samples/cjs/empty-top-levels/component.ractive.html`).then(r => r.text()).then(source => {
    const srcName = 'component.ractive.html'
    const destName = 'component.js'
    const parts = toParts(source)
    const resolver = id => { throw new Error('THIS SHOULD NEVER BE CALLED') }
    const constructor = toConstructor({ srcName, destName, parts, resolver })
...
QUnit.test(`links-only`, assert => {
  return fetch(`/base/test/samples/cjs/links-only/component.ractive.html`).then(r => r.text()).then(source => {
    const dependencies = {
      './path/to/MyComponent.ractive.html': Ractive.extend({ template: '<div class="my-component">My Component</div>' }),
      './path/to/MyOtherComponent.ractive.html': Ractive.extend({ template: '<div class="my-other-component">My Other Component</div>' }),
      './path/to/YetAnotherComponent.ractive.html': Ractive.extend({ template: '<div class="yet-another-component">Yet Another Component</div>' })
...
QUnit.test(`reference`, assert => {
  return fetch(`/base/test/samples/cjs/reference/component.ractive.html`).then(r => r.text()).then(source => {
    const dependencies = {
      'hhgttg': 42,
      './path/to/MyComponent.ractive.html': Ractive.extend({ template: '<div class="my-component">My Component</div>' }),
      './path/to/MyOtherComponent.ractive.html': Ractive.extend({ template: '<div class="my-other-component">My Other Component</div>' }),
...
QUnit.test(`reordered`, assert => {
  return fetch(`/base/test/samples/cjs/reordered/component.ractive.html`).then(r => r.text()).then(source => {
    const dependencies = {
      'hhgttg': 42,
      './path/to/MyComponent.ractive.html': Ractive.extend({ template: '<div class="my-component">My Component</div>' }),
      './path/to/MyOtherComponent.ractive.html': Ractive.extend({ template: '<div class="my-other-component">My Other Component</div>' }),
...
QUnit.test(`script-only`, assert => {
  return fetch(`/base/test/samples/cjs/script-only/component.ractive.html`).then(r => r.text()).then(source => {
    const dependencies = {
      'hhgttg': 42
    }
    const srcName = 'component.ractive.html'
...
QUnit.test(`script-with-mustache`, assert => {
  return fetch(`/base/test/samples/cjs/script-with-mustache/component.ractive.html`).then(r => r.text()).then(source => {
    const srcName = 'component.ractive.html'
    const destName = 'component.js'
    const parts = toParts(source)
    const resolver = id => { throw new Error('THIS SHOULD NEVER BE CALLED') }
...
QUnit.test(`style-only`, assert => {
  return fetch(`/base/test/samples/cjs/style-only/component.ractive.html`).then(r => r.text()).then(source => {
    const srcName = 'component.ractive.html'
    const destName = 'component.js'
    const parts = toParts(source)
    const resolver = id => { throw new Error('THIS SHOULD NEVER BE CALLED') }
...
QUnit.test(`style-with-mustache`, assert => {
  return fetch(`/base/test/samples/cjs/style-with-mustache/component.ractive.html`).then(r => r.text()).then(source => {
    const srcName = 'component.ractive.html'
    const destName = 'component.js'
    const parts = toParts(source)
    const resolver = id => { throw new Error('THIS SHOULD NEVER BE CALLED') }
...
QUnit.test(`template-only`, assert => {
  return fetch(`/base/test/samples/cjs/template-only/component.ractive.html`).then(r => r.text()).then(source => {
    const srcName = 'component.ractive.html'
    const destName = 'component.js'
    const parts = toParts(source)
    const resolver = id => { throw new Error('THIS SHOULD NEVER BE CALLED') }
