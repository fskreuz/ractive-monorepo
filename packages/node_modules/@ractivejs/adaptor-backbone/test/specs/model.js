...
import { backboneModel } from '@ractivejs/adaptor-backbone'

QUnit.module('ractive-adaptor-backbone models')

QUnit.test('Adaptor can detect models', t => {
  t.ok(backboneModel.filter(new Backbone.Model()))
})

QUnit.test('Initialize with pre-filled model', t => {
...
QUnit.test('Initialize with empty model', t => {
...
QUnit.test('Initialize with set model', t => {
  const model = new Backbone.Model({ message: 'hello' })
  const instance = Ractive({
    adapt: [ backboneModel ],
    template: `{{ model.message }}`
...
QUnit.test('Updating via keypath', t => {
  const model = new Backbone.Model({ message: 'hello ' })
  const instance = Ractive({
    adapt: [ backboneModel ],
    data: { model },
...
QUnit.test('Updating via model', t => {
  const model = new Backbone.Model({ message: 'hello' })
  const instance = Ractive({
    adapt: [ backboneModel ],
    data: { model },
...
QUnit.test('Updating keypath triggers model listeners', t => {
  const model = new Backbone.Model({ message: 'hello' })
  const instance = Ractive({
    adapt: [ backboneModel ],
    data: { model }
...
QUnit.test('Updating model triggers observers', t => {
  const model = new Backbone.Model({ message: 'hello' })
  const instance = Ractive({
    adapt: [ backboneModel ],
    data: { model }
...
QUnit.test('Resetting model with a POJO', t => {
  const model = new Backbone.Model({ message: 'hello' })
  const instance = Ractive({
    adapt: [ backboneModel ],
    data: { model },
...
QUnit.test('Resetting model with a new model', t => {
...
QUnit.test('Resetting model with a non-adapted value', t => {
  const model = new Backbone.Model({ message: 'hello' })
  const instance = Ractive({
    adapt: [ backboneModel ],
    data: { model },
...
QUnit.test('Override model.toJSON to customize POJO representation', t => {
