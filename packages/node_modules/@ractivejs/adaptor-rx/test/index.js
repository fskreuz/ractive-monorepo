/* eslint-env browser, qunit */
import Ractive from '@ractivejs/core'
import { of, Subject, BehaviorSubject } from '@reactivex/rxjs/dist/esm2015'
import {rx} from '@ractivejs/adaptor-rx'

QUnit.module('adaptor-rx')

QUnit.test('Adaptor can detect an observable', t => {
  t.ok(rx.filter(of('foo')))
  t.ok(rx.filter(new Subject()))
  t.ok(rx.filter(new BehaviorSubject('foo')))
})

QUnit.test('Initialize using a regular Observable', t => {
  const observable = of('qux')
  const instance = Ractive({
    adapt: [ rx ],
    data: { observable },
    template: `{{ observable }}`
  })

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'qux')
  t.strictEqual(instance.toHTML(), 'qux')
})

QUnit.test('Initialize using a Subject', t => {
  const observable = new Subject()
  const instance = Ractive({
    adapt: [ rx ],
    data: { observable },
    template: `{{ observable }}`
  })

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), undefined)
  t.strictEqual(instance.toHTML(), '')
})

QUnit.test('Initialize using a BehaviorSubject', t => {
  const observable = new BehaviorSubject('qux')
  const instance = Ractive({
    adapt: [ rx ],
    data: { observable },
    template: `{{ observable }}`
  })

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'qux')
  t.strictEqual(instance.toHTML(), 'qux')
})

QUnit.test('Initialize using a set regular Observable', t => {
  const observable = of('qux')
  const instance = Ractive({
    adapt: [ rx ],
    template: `{{ observable }}`
  })

  instance.set('observable', observable)

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'qux')
  t.strictEqual(instance.toHTML(), 'qux')
})

QUnit.test('Initialize using a set Subject', t => {
  const observable = new Subject()
  const instance = Ractive({
    adapt: [ rx ],
    template: `{{ observable }}`
  })

  instance.set('observable', observable)

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), undefined)
  t.strictEqual(instance.toHTML(), '')
})

QUnit.test('Initialize using a set BehaviorSubject', t => {
  const observable = new BehaviorSubject('qux')
  const instance = Ractive({
    adapt: [ rx ],
    template: `{{ observable }}`
  })

  instance.set('observable', observable)

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'qux')
  t.strictEqual(instance.toHTML(), 'qux')
})

QUnit.test('Subject updates Ractive', t => {
  const observable = new Subject()
  const instance = Ractive({
    adapt: [ rx ],
    data: { observable },
    template: `{{ observable }}`
  })

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), undefined)
  t.strictEqual(instance.toHTML(), '')

  observable.next('foo')

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'foo')
  t.strictEqual(instance.toHTML(), 'foo')
})

QUnit.test('BehaviorSubject updates Ractive', t => {
  const observable = new BehaviorSubject('qux')
  const instance = Ractive({
    adapt: [ rx ],
    data: { observable },
    template: `{{ observable }}`
  })

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'qux')
  t.strictEqual(instance.toHTML(), 'qux')

  observable.next('foo')

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'foo')
  t.strictEqual(instance.toHTML(), 'foo')
})

QUnit.test('Ractive should not update Observable', t => {
  const observable = of('qux')
  const instance = Ractive({
    adapt: [ rx ],
    data: { observable },
    template: `{{ observable }}`
  })

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'qux')
  t.strictEqual(instance.toHTML(), 'qux')

  instance.set('observable', 'foo')

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'qux')
  t.strictEqual(instance.toHTML(), 'qux')
})

QUnit.test('Ractive updates Subject', t => {
  const observable = new Subject()
  const instance = Ractive({
    adapt: [ rx ],
    data: { observable },
    template: `{{ observable }}`
  })

  observable.subscribe(v => t.strictEqual(v, 'foo'))

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), undefined)
  t.strictEqual(instance.toHTML(), '')

  instance.set('observable', 'foo')

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'foo')
  t.strictEqual(instance.toHTML(), 'foo')
})

QUnit.test('Ractive updates BehaviorSubject', t => {
  const observable = new BehaviorSubject('qux')
  const instance = Ractive({
    adapt: [ rx ],
    data: { observable },
    template: `{{ observable }}`
  })

  const results = ['qux', 'foo']
  let count = 0

  observable.subscribe(v => t.strictEqual(v, results[count++]))

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'qux')
  t.strictEqual(instance.toHTML(), 'qux')

  instance.set('observable', 'foo')

  t.strictEqual(instance.get('observable'), observable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'foo')
  t.strictEqual(instance.toHTML(), 'foo')
})

QUnit.test('Resetting with an Observable', t => {
  const oldObservable = of('hello')
  const newObservable = of('world')
  const instance = Ractive({
    adapt: [ rx ],
    data: { observable: oldObservable },
    template: `{{ observable }}`
  })

  t.strictEqual(instance.get('observable'), oldObservable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'hello')
  t.strictEqual(instance.toHTML(), 'hello')

  instance.set('observable', newObservable)

  t.strictEqual(instance.get('observable'), newObservable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'world')
  t.strictEqual(instance.toHTML(), 'world')
})

QUnit.test('Resetting with an Observer', t => {
  const oldObservable = new BehaviorSubject('hello')
  const newObservable = new BehaviorSubject('world')
  const instance = Ractive({
    adapt: [ rx ],
    data: { observable: oldObservable },
    template: `{{ observable }}`
  })

  const results = ['world', 'bar', 'qux']
  let count = 0

  newObservable.subscribe(v => t.strictEqual(v, results[count++]))

  t.strictEqual(instance.get('observable'), oldObservable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'hello')
  t.strictEqual(instance.toHTML(), 'hello')

  instance.set('observable', newObservable)

  t.strictEqual(instance.get('observable'), newObservable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'world')
  t.strictEqual(instance.toHTML(), 'world')

  oldObservable.next('foo')
  newObservable.next('bar')

  t.strictEqual(instance.get('observable'), newObservable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'bar')
  t.strictEqual(instance.toHTML(), 'bar')

  instance.set('observable', 'qux')

  t.strictEqual(instance.get('observable'), newObservable)
  t.strictEqual(instance.get('observable', { unwrap: false }), 'qux')
  t.strictEqual(instance.toHTML(), 'qux')
})
